### LORA Fine-Tuning Distributed for Chunked PEFT Distributed
### modified from https://github.com/pytorch/torchtune/blob/v0.4.0/recipes/lora_finetune_distributed.py
...
class LoRAFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.seed = training.set_seed(seed=cfg.seed, debug_mode=cfg.debug_mode) # [chunked peft] add debug_mode
        self.epochs_run = 0
    ...
    def _setup_data(
        ...
        # NOTE: [chunked peft] 1. only keep first 100000 samples for test efficiency
        #                      2. only keep long-enough samples (# tokens == seq_len after trunc) for throughput testing
        if self._is_rank_zero:
            print(f"# valid samples: {len(ds._data)}")
    ...
    def chunked_peft_forward(self, batch, chunk_size, fwd_chunk_timings) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        while num_processed_tokens < num_total_tokens:
            ...
            tokens: torch.Tensor = batch["tokens"][:, num_processed_tokens: num_will_complete_tokens]
            cur_batch: Dict = {
                "tokens": tokens,
                "mask": mask,
            ...
            batch_to_device(cur_batch, self._device)
            
            with self.activations_handling_ctx:
                cur_logits, res_past_key_values = self._model(**cur_batch)
            
            if isinstance(cur_logits, list):
                cur_logits = cur_logits[0]

            # NOTE: detach prev tensor from the computation graph of new chunks
            past_key_values = res_past_key_values
            ...
            if num_will_complete_tokens < num_total_tokens:
                ...
                shift_targets: torch.Tensor = labels[:, num_processed_tokens+1: num_will_complete_tokens+1] \
                                                    .to(device=self._device)
            else: # last chunk, ignore the last pos of logits
                shift_logits:  torch.Tensor = cur_logits[..., :-1, :].contiguous()
                shift_targets: torch.Tensor = labels[:, num_processed_tokens+1: num_will_complete_tokens] \
                                                    .to(device=self._device)
            ...
            shift_targets = shift_targets.to(self._device)
            # cur_loss = self._loss_fn(shift_logits, shift_targets) * num_cur_step
            cur_loss = fixed_cross_entropy(shift_logits, shift_targets, 1, self._loss_fn.ignore_index)

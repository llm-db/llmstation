### instruct_dataset w/ long-enough-sentence for Chunked Distributed PEFT throughput test
### Modified from https://github.com/pytorch/torchtune/blob/v0.4.0/torchtune/datasets/_sft.py
### and https://github.com/pytorch/torchtune/blob/v0.4.0/torchtune/datasets/_instruct.py
...
class PreprocessedSFTDataset(Dataset):
    ...
    def __init__(
        ...
        **load_dataset_kwargs: Dict[str, Any],
        ...
        # FIXME: [chunked fine-tuning] select, tokenize, post-filter
        post_filter_fn = lambda sample: len(sample["tokens"]) >= model_transform.max_seq_len
        self._data = self._data.select(range(100000))\
                               .map(lambda sample: self._prepare_sample(sample))\
                               .filter(post_filter_fn)
...
def instruct_dataset_preprocessed(
    ...
    *,
    ...
    split: str = "train",
    **load_dataset_kwargs: Dict[str, Any],
) -> Union[PreprocessedSFTDataset, PackedDataset]:
    """

...
Same as hf_coserving_v0.py (alternating decode + LoRA train, 2 adapters) but uses:
...
# Same 2 adapters as v0: ft (train) + infer (decode)
...
# --------------------------------------------------------------------------
# Prefill (step 0 decode, uncompiled)
# --------------------------------------------------------------------------
print("[prefill] Running uncompiled prefill (infer adapter)...")
...
print(f"[prefill] Done. Cache has {past_kv.get_seq_length()} tokens.")
...
for step in range(max_new_tokens):
    if step > 0:
        # --- Decode (infer adapter, KV cache, compiled) ---
        pos = prompt_len + step - 1
        position_ids = torch.tensor([[pos]], device=device)

        model_compiled.set_adapter("infer")
        model_compiled.eval()
    ...
    # --- Train forward (ft adapter, compiled) ---

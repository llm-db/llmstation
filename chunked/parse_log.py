...
def main():
    ...
    parser.add_argument(
        "--chunk-size",
        type=int,
        help="chunk size of chunked-training")
    parser.add_argument(
        "--nproc-per-node",
        type=int,
        default=2,
    ...
    with open(args.result_path, 'a', encoding='utf-8') as f:
        ...
        print("{:<40} {:<10.2f}".format("PEFT throughput (sample/s):",
            avg_tps * args.nproc_per_node), file=f)
        print("{:<40} {:<10}".format("Chunk size:", args.chunk_size), file=f)

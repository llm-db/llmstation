...
# $2 - LoRA distribution
# $3 - LoRA rank
# $4 - chunk size of chunked-training

if [[ $# -lt 4 ]]; then
    echo "Too few arguments. "
    echo "Usage $0 <download dir> <LoRA distribution> <LoRA rank> <chunk size>"
    exit
fi
...
torchtune_log=fig13_torchtune.log
...
nohup tune run --nproc_per_node 2 lora_finetune_distributed --config llama3_1/8B_lora \
	...
	tokenizer.path="${download_dir}/Meta-Llama-3.1-8B/original/tokenizer.model" tokenizer.max_seq_len=$chunk_size \
...
python ../parse_log.py --vllm-log $vllm_client_log --torchtune-log $latest_torchtune_log \
	--chunk-size $chunk_size --start-line $start_line --end-line $end_line
